...
import {userService, reportProfileService} from "@/services"
...
export const GET = auth(async (req) => {
  ...
  if (myUser && myUser.role === "admin") {
    const result = await reportProfileService.findAdminReportProfilesPaginatedAndSorted(parsedPage)
    const response = mapReportProfilesToResponse(result)

    return NextResponse.json(response)
  }
})

function mapReportProfilesToResponse(reportProfiles: {
  username: string,
  reason: string,
  user: string,
}[]) {
  return reportProfiles.map(({username, reason, user}) => ({
    username,
    reason,
    user
  }))
