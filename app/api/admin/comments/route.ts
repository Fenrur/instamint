...
export const GET = auth(async (req) => {
  ...
  if (myUser && myUser.role === "admin") {
    const result = await commentService.findAdminCommentsPaginatedAndSorted(parsedPage)
    const response = mapCommentsToResponse(result)

    return NextResponse.json(response)
  }
})

function mapCommentsToResponse(comments: {
  id: number,
  commentary: string,
  ownerUsername: string,
  ownerEmail: string
}[]) {
  return comments.map(({id, commentary, ownerUsername, ownerEmail}) => ({
    id,
    commentary,
    ownerUsername,
    ownerEmail
