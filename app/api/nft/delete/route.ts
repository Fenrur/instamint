...
import {profileService, nftService} from "@/services"
import {isContentType} from "@/http/content-type"
import {DeleteNftResponse, DeleteNftRequest} from "@/http/rest/types"
...
export const DELETE  = auth(async (req) => {
  ...
  const bodyParsedResult = DeleteNftRequest.safeParse(await req.json())
  ...
  const result = await nftService.deleteNftById(String(id))
  ...
  switch(result) {
    ...
    case "deleted":
      return NextResponse.json({deleted: true} satisfies DeleteNftResponse)
  }
})
