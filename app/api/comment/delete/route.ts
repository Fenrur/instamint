...
import {profileService, commentService} from "@/services"
import {isContentType} from "@/http/content-type"
import {DeleteCommentResponse, DeleteCommentRequest} from "@/http/rest/types"
...
export const DELETE  = auth(async (req) => {
  ...
  const bodyParsedResult = DeleteCommentRequest.safeParse(await req.json())
  ...
  const result = await commentService.deleteCommentById(String(id))
  ...
  switch(result) {
    ...
    case "deleted":
      return NextResponse.json({deleted: true} satisfies DeleteCommentResponse)
  }
})
